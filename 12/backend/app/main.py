...
async def chat_endpoint(request: Request, payload: ChatRequest, user=Depends(verify_user)):
    try:
        ...
        # Fixed Session Creation (using client.chats.create instead of model.start_chat)
        ...
        for attempt in range(MAX_RETRIES + 1):
            ...
            all_valid, error_msg = True, ""
            
            if code_blocks:
                for code in code_blocks:
                    val_error = validate_python_code(code)
                    if val_error:
                        all_valid, error_msg = False, val_error
                        break
            
            if all_valid:
                return {"response": text_response, "validated": len(code_blocks) > 0}
            ...
            if attempt < MAX_RETRIES:
                ...
                return {"response": text_response + "\n\n*(Note: FRIDAY detected potential syntax errors in this code. Please review carefully.)*", "validated": False}
