...
const ChatPage = () => {
  ...
  // Keep sidebar state in sync with actual viewport width on resize/rotation
  useEffect(() => {
    const handleResize = () => {
  ...
  // Lock body scroll while the mobile overlay/sidebar is open
  useEffect(() => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (res.status === 429) {
        throw new Error(
          "Rate limit exceeded. You are sending requests too quickly.",
        );
      }
      if (res.status === 401) {
        ...
        .insert([
          {
            ...
            is_validated: data.validated || false,
          },
        ])
        .select()
        .single();
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: Date.now().toString(),
          role: "assistant",
          content: `**Error:** ${error.message || "Could not reach FRIDAY Backend."}`,
        },
      ]);
    } finally {
      setIsGenerating(false);
      ...
      <main className="flex-1 flex flex-col h-full relative overflow-hidden">
        ...
        >
          ...
          >
            ...
            {isGenerating && (
              <div
                className="flex items-center gap-3 text-sm text-friday-accent animate-pulse"
                aria-label="FRIDAY is generating a response"
              >
